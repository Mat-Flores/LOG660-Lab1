...
/**
 * Parcourt les fichiers XML et transmet chaque fiche a EcritureBD.
 * Le flux est ouvert et ferme par GestionFlux.
 */
public class LectureXML {
    ...
    private final EcritureBD ecriture;

    public LectureXML(EcritureBD ecriture) {
        this.ecriture = ecriture;
    }

    public void lirePersonnes(String nomFichier) {
        if (!ecriture.estConnectee()) {
            return;
        }
        InputStream flux = null;
        try {
            flux = GestionFlux.ouvrirFichier(nomFichier);
            XmlPullParser parser = GestionFlux.nouveauParser(flux);
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (parser.getName().equals("personne") && id >= 0) {
                        ecriture.insererPersonne(id, nom, anniversaire, lieu, photo, bio);
                        id = -1;
                        nom = null;
                        anniversaire = null;
                        lieu = null;
            ...
            ecriture.terminerLotPersonnes();
            GestionFlux.fermer(flux);
            ecriture.afficherPersonnes();
        }
    }

    public void lireFilms(String nomFichier) {
        if (!ecriture.estConnectee()) {
            return;
        }
        InputStream flux = null;
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (parser.getName().equals("film") && id >= 0) {
                        ecriture.insererFilm(id, titre, annee, pays, langue,
            ...
            ecriture.terminerLotsFilms();
            GestionFlux.fermer(flux);
            ecriture.afficherFilms();
        }
    }

    public void lireClients(String nomFichier) {
        if (!ecriture.estConnectee()) {
            return;
        }
        InputStream flux = null;
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (parser.getName().equals("client") && id >= 0) {
                        ecriture.insererClient(id, nomFamille, prenom, courriel, tel,
            ...
            ecriture.terminerLotClients();
            GestionFlux.fermer(flux);
            ecriture.afficherClients();
        }
    }
}
